...
def _julia_depot_impl(rctx):
    ...
    # bundled depots on the path; without it Pkg is recompiled into the fresh depot, see
    # instantiate.sh. The directory is created here so a hook can write into it.
    if rctx.attr.depot:
        depot_path = _expand_depot(rctx, rctx.attr.depot)
        env["JULIA_DEPOT_PATH"] = depot_path + ":"
        res = rctx.execute(["mkdir", "-p", depot_path])
        if res.return_code != 0:
            fail("julia_depot: cannot create depot {}:\n{}".format(depot_path, res.stderr))
    ...
    # the launching environment set one or the rule declared one: an unset depot stays
    # unset so Julia applies its own default rather than being handed "". The path is
    # genuinely environmental and therefore the one thing that legitimately varies
    # between machines.
...
julia_depot = repository_rule(
    ...
    attrs = {
        ...
        "depot": attr.string(
            doc = "The depot to instantiate into, overriding JULIA_DEPOT_PATH. {HOME} and {USER} expand " +
                  "from the fetch environment. Exported through env.sh with a trailing separator so " +
                  "Julia's bundled depots stay on the path.",
        ),
        "hook": attr.label(
            allow_single_file = True,
